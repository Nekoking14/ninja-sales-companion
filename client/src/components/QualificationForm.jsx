...
export default function QualificationForm({ session, prospect }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', background: 'var(--bg)', position: 'relative' }}>
        ...
        {/* MSP Do Not Book popup */}
        {showMspDNB && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(4,6,18,0.78)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300 }}>
            <div style={{ background: '#0F1628', border: '2px solid var(--amber)', borderRadius: 16, padding: '32px 28px', maxWidth: 360, textAlign: 'center', boxShadow: '0 24px 60px rgba(0,0,0,0.6)' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>🔒</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--amber)', marginBottom: 10 }}>MSP — Do Not Book</div>
              <div style={{ fontSize: 13, color: 'var(--txt2)', lineHeight: 1.6, marginBottom: 24 }}>
                They plan to keep using their MSP. Do not book a demo — there is no direct path to NinjaOne at this time.
              </div>
              <button onClick={() => setShowMspDNB(false)} style={{ background: 'var(--amber2)', color: 'var(--amber)', border: '1px solid var(--amber)', borderRadius: 9, padding: '10px 28px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                Understood
              </button>
            </div>
          </div>
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  {form.mspInhouse === 'yes' && (
                    <div style={{ marginTop: 8 }}>
                      ...
                      <textarea
                        className="input"
                        value={form.mspInhouseWhy}
                        onChange={e => set('mspInhouseWhy', e.target.value)}
                        placeholder="Notes on why they want to bring IT in-house…"
                        rows={3}
                        style={{ fontSize: 13, padding: '9px 12px', resize: 'vertical', minHeight: 72, lineHeight: 1.5 }}
                      />
                    </div>
                  )}
                </div>
          ...
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div>
                ...
                <input className="input" value={form.mobileIos} onChange={e => set('mobileIos', e.target.value)} placeholder="e.g. 50" style={{ fontSize: 13, padding: '9px 12px' }} />
              </div>
              <div>
                <div style={{ fontSize: 10, color: 'var(--txt3)', marginBottom: 4 }}>Android devices</div>
                <input className="input" value={form.mobileAndroid} onChange={e => set('mobileAndroid', e.target.value)} placeholder="e.g. 30" style={{ fontSize: 13, padding: '9px 12px' }} />
              </div>
            </div>
          </div>
          <div style={{ marginTop: 10 }}>
...
function Label({ text }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--txt3)', marginBottom: 5 }}>{text}</div>
  )
}
      ...
      {options.map(opt => {
        ...
        return (
          <button
            ...
            style={{ flex: 1, padding: '8px 10px', borderRadius: 8, fontSize: 12, fontWeight: active ? 600 : 400, background: active ? opt.bg : 'var(--card2)', color: active ? opt.color : 'var(--txt2)', border: `1px solid ${active ? opt.brd : 'var(--brd2)'}`, transition: 'all 0.12s', cursor: 'pointer' }}
          >
            {opt.label}
          </button>
        )
