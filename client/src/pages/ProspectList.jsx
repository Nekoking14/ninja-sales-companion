...
const PERSONA_COLORS = {
  ...
  'C-Level': 'var(--coral)', 'Technician': 'var(--green)'
}
...
function generateExport (session, persona) {
  ...
  const qualLines = q ? [
    ...
    line('In-house plan',  q.mspInhouse === 'yes' ? `Yes — ${q.mspInhouseWhy || ''}`.trim() : q.mspInhouse === 'no' ? 'No' : null),
  ...
  // SPIN notes — three sections
  const spinLines = [
    q?.notesSituation?.trim()   ? ['', '', 'SITUATION', sep, ...q.notesSituation.trim().split('\n').map(l => `  ${l}`)]   : null,
    q?.notesPain?.trim()        ? ['', '', 'PAIN', sep, ...q.notesPain.trim().split('\n').map(l => `  ${l}`)]             : null,
    q?.notesImplication?.trim() ? ['', '', 'IMPLICATION', sep, ...q.notesImplication.trim().split('\n').map(l => `  ${l}`)] : null,
  ].filter(v => v != null).flat()
  // Legacy single notes field fallback
  const noteLines = q?.notes?.trim() && !q?.notesSituation && !q?.notesPain ? [
    '', '', 'NOTES', sep,
    ...q.notes.trim().split('\n').map(l => `  ${l}`)
  ] : []
  ...
  return [
    ...
    line('Use case',        Array.isArray(q?.useCase) ? q.useCase.join(', ') : (q?.useCase || null)),
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      ...
      <div>
        ...
        {[
          { key: 'notesSituation',   label: 'S — Situation',   color: 'var(--blue)'  },
          { key: 'notesPain',        label: 'P — Pain',        color: 'var(--amber)' },
          { key: 'notesImplication', label: 'I — Implication', color: 'var(--red)'   },
        ].filter(({ key }) => q[key]?.trim()).map(({ key, label, color }) => (
          <div key={key} style={{ marginTop: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color, marginBottom: 5 }}>{label}</div>
            <div style={{ fontSize: 12, color: 'var(--txt2)', lineHeight: 1.6, background: 'var(--card)', padding: '8px 10px', borderRadius: 8, whiteSpace: 'pre-wrap' }}>{q[key]}</div>
          </div>
        ))}
        {/* Legacy notes fallback */}
        {q.notes && !q.notesSituation && !q.notesPain && (
          <div style={{ marginTop: 14 }}>
            <div className="section-label" style={{ marginBottom: 6 }}>Notes</div>
            <div style={{ fontSize: 12, color: 'var(--txt2)', lineHeight: 1.5, background: 'var(--card)', padding: '8px 10px', borderRadius: 8 }}>{q.notes}</div>
          </div>
        )}
      </div>
    </div>
                        ...
                        return (
                          <>
                            ...
                            {sd && (() => {
                              ...
                              const score = computeCallScore(qual)
                              return (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 24 }}>
                              ...
                              {/* SPIN Notes */}
                              <div>
                                <div className="section-label" style={{ marginBottom: 12 }}>SPIN Notes</div>
                                {(() => {
                                  let sq = null
                                  try { sq = sd?.qualification_data ? JSON.parse(sd.qualification_data) : null } catch {}
                                  const spinSections = [
                                    { key: 'notesSituation',   label: 'S — Situation',   color: 'var(--blue)'  },
                                    { key: 'notesPain',        label: 'P — Pain',        color: 'var(--amber)' },
                                    { key: 'notesImplication', label: 'I — Implication', color: 'var(--red)'   },
                                  ].filter(s => sq?.[s.key]?.trim())
                                  if (spinSections.length > 0) {
                                    return spinSections.map(({ key, label, color }) => (
                                      <div key={key} style={{ marginBottom: 12 }}>
                                        <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color, marginBottom: 5 }}>{label}</div>
                                        <div style={{ background: 'var(--card)', border: `1px solid ${color}40`, borderRadius: 9, padding: '10px 12px', fontSize: 12, lineHeight: 1.6, color: 'var(--txt)', whiteSpace: 'pre-wrap' }}>{sq[key]}</div>
                                      </div>
                                    ))
                                  }
                                  // Legacy single notes fallback
                                  const legacy = sq?.notes
                                  return legacy ? (
                                    <div style={{ background: 'var(--card)', border: '1px solid var(--brd)', borderRadius: 9, padding: '10px 12px', fontSize: 12, lineHeight: 1.6, color: 'var(--txt)', whiteSpace: 'pre-wrap' }}>{legacy}</div>
                                  ) : (
